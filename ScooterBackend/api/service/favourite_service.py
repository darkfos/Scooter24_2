...
class FavouriteService:
    ...
    async def create_favourite_product(
        ...
        is_created: bool = await FavouriteRepository(session=session).add_one(
            data=Favourite(
    ...
    @staticmethod
    async def get_all_favourite_product_by_user_id(session: AsyncSession, token: str) -> FavouriteBase:
        """
        ...
        all_favourite_products: Union[List, List[Favourite]] = await FavouriteRepository(session=session).get_all_data_for_id_user(id_user=jwt_data.get("id_user"))
    ...
    async def get_information_about_fav_product_by_id(
        ...
        if is_admin:
            find_favourite_product: Union[Favourite, None] = \
                await FavouriteRepository(session=session).get_all_data_for_favourite_product_by_id(id_fav_product=id_fav_product)
    ...
    async def get_all_favourites(
        ...
        if is_admin:
            all_fav: Union[List, List[Favourite]] =  await FavouriteRepository(session=session).find_all()
            if all_fav:
    ...
    async def delete_favourite_product(
        ...
        fav_rep = FavouriteRepository(session=session)
        get_favourite_data: Union[None, Favourite] = await fav_rep.find_one(other_id=id_favourite)

        if get_favourite_data:
            get_favourite_data = get_favourite_data[0]

            if get_favourite_data.id_user == jwt_data.get("id_user"):
                is_deleted: bool = await fav_rep.delete_one(other_id=id_favourite)

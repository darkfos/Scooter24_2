...
from src.api.core.product_catalog.error.http_product_exception import ProductHttpError
...
from src.api.core.product_catalog.schemas.product_dto import ListProductBase, ProductBase, ProductAllInformation, ProductIsCreated, UpdateProduct, UpdateProductDiscount, UploadFile
...
class ProductService:
    ...
    @staticmethod
    async def create_product(engine: IEngineRepository, token: str, new_product: ProductBase, photo_product: UploadFile, token_data: dict = dict()) -> ProductIsCreated:
        """
        ...
        async with engine:
            ...
            if is_admin:
                ...
                # Create product
                product_is_created: bool = await engine.product_repository.add_one(data=product)
                ...
                if product_is_created:
                    ...
                    if url_save_photo:
                        ...
                        if is_updated:
                            return ProductIsCreated(is_created=True, product_name=url_save_photo)

                    raise HTTPException(
                        status_code=status.HTTP_409_CONFLICT,
                        detail="Не удалось загрузить фотографию продукта",
                    )
            logging.critical(msg=f"{ProductService.__name__} " f"Не удалось создать новый продукт")
            await ProductHttpError().http_failed_to_create_a_new_product()

    @auth(worker=AuthenticationEnum.DECODE_TOKEN.value)
    @staticmethod
    async def add_new_category(engine: IEngineRepository, id_product: int, id_category: int, admin_data: str, token_data: dict = dict()) -> None:
        """
        ...
        logging.info(msg=f"{ProductService.__name__} " f"Добавление новой категории для товара")
        ...
        async with engine:
            ...
            if is_admin:
                created_new_category_for_product = await engine.product_category_repository.add_new_category(id_category=id_category, id_product=id_product)
                return True if created_new_category_for_product else (await CategoryHttpError().http_failed_to_create_a_new_category())
            logging.critical(msg=f"{ProductService.__name__} " f"Не удалось добавить новую " f"категорию к товару")
            await UserHttpError().http_user_not_found()

    @redis
    @staticmethod
    async def get_all_products(engine: IEngineRepository, redis_search_data: str) -> ListProductBase:
        """
    ...
    async def get_products_by_category(
        ...
        logging.info(msg=f"{ProductService.__name__} " f"Получение списка товаров по категории " f"category_data={category_data}")
        async with engine:
            all_products: Union[List, List[Product]] = await engine.product_repository.find_by_category(how_to_find=(category_data if not category_data.isdigit() else int(category_data)))
    ...
    async def find_product_by_id(engine: IEngineRepository, id_product: int) -> ProductBase:
        ...
        logging.info(msg=f"{ProductService.__name__} " f"Поиск продукта по id={id_product}")
        async with engine:
            ...
            if product_data:
                return ProductBase(
                    ...
                    date_update_information=product_data[0].date_update_information,  # noqa
            ...
            logging.critical(msg=f"{ProductService.__name__} " f"Не удалось удалить продукт по " f"id={id_product}")
    ...
    async def find_product_by_name(engine: IEngineRepository, name_product: str) -> ProductBase:
        ...
        logging.info(msg=f"{ProductService.__name__} " f"Поиск продукта по name={name_product}")
        async with engine:
            product_data: Union[None, Product] = await engine.product_repository.find_product_by_name(name_product=name_product)
            ...
            logging.critical(msg=f"{ProductService.__name__} " f"Не удалось найти продукт")
            await ProductHttpError().http_product_not_found()
    ...
    async def product_is_created(engine: IEngineRepository, product_name: str) -> None:
        ...
        logging.info(msg=f"{ProductService.__name__} " f"Проверка существования продукта")
        async with engine:
    ...
    @redis
    @staticmethod
    async def get_all_information_about_product(engine: IEngineRepository, token: str, id_product: int, redis_search_data: str, token_data: dict = dict()) -> ProductAllInformation:
        """
        ...
        logging.info(msg=f"{ProductService.__name__} " f"Получение полной информации о продукте")
            ...
            if is_admin:
                ...
                if product_data:
                    return ProductAllInformation(
                        ...
                        photo_product=f"{product_data[0].photo_product}",
                        price_discount=(product_data[0].product_discount if product_data[0].product_discount else 0),
                        reviews=[review_p.read_model() for review_p in product_data[0].reviews],
                        orders=[order_pr.read_model() for order_pr in product_data[0].order],  # noqa
                        favourites=[fav_p.read_model() for fav_p in product_data[0].product_info_for_fav],
                        categories=[cat_data.read_model() for cat_data in product_data[0].product_all_categories],  # noqa
                    )
                logging.critical(msg=f"{ProductService.__name__} " f"Не удалось найти продукт")
                await ProductHttpError().http_product_not_found()
            logging.critical(msg=f"{ProductService.__name__} " f"Не удалось найти продукт, " f"пользователь не был найден")
            await UserHttpError().http_user_not_found()

    @auth(worker=AuthenticationEnum.DECODE_TOKEN.value)
    @staticmethod
    async def update_information(engine: IEngineRepository, id_product: int, token: str, data_to_update: UpdateProduct, token_data: dict = dict()) -> None:
        """
        ...
        async with engine:
            # Is admin
            is_admin: bool = await engine.user_repository.find_user_by_email_and_password(email=token_data.get("email"))

            if is_admin and is_admin.id_type_user == 2:
                # Update data
                is_updated: bool = await engine.product_repository.update_one(other_id=id_product, data_to_update=data_to_update.model_dump())

                if is_updated:
                    return
                logging.critical(msg=f"{ProductService.__name__} " f"Не удалось обновить продукт," f" продукт не был найден")
                (await ProductHttpError().http_failed_to_update_product_information())
            logging.critical(msg=f"{ProductService.__name__} " f"Не удалось обновить продукт," f" пользователь не был найден")
            await UserHttpError().http_user_not_found()

    @auth(worker=AuthenticationEnum.DECODE_TOKEN.value)
    @staticmethod
    async def update_photo(engine: IEngineRepository, token: str, photo_data: str, product_id: int, token_data: dict = dict()) -> None:
        """
        ...
        async with engine:
            ...
            if is_admin:
                ...
                return
            logging.critical(msg=f"{ProductService.__name__} " f"Не удалось обновить фотографию")
            await UserHttpError().http_user_not_found()

    @auth(worker=AuthenticationEnum.DECODE_TOKEN.value)
    @staticmethod
    async def delete_product_by_id(engine: IEngineRepository, token: str, id_product: int, token_data: dict = dict()) -> None:
        """
        ...
        logging.info(msg=f"{ProductService.__name__} " f"Удаление продукта по id = {id_product}")
        ...
        async with engine:
            ...
            logging.info(msg=f"{ProductService.__name__} " f"Не удалось удалить продукт по " f"id = {id_product}, " f"пользователь не был найден")
            await UserHttpError().http_user_not_found()
    ...
    @redis
    @staticmethod
    async def get_recommended_products(engine: IEngineRepository, redis_search_data: str) -> Union[List, List[ProductBase]]:
        """
        Метод сервиса для получения рекомендованных товаров.
        :session:
        """

        logging.info(msg=f"{ProductService.__name__} " f"Получение рекомендованных товаров")
                ...
                while len(result) != 7:
                    ...
                    result.append(
                        ProductBase(
                            ...
                            explanation_product=rnd_product[0].explanation_product,  # noqa
                            ...
                            date_create_product=rnd_product[0].date_create_product,  # noqa
    ...
    @auth(worker=AuthenticationEnum.DECODE_TOKEN.value)
    @staticmethod
    async def update_product_discount(engine: IEngineRepository, token: str, id_product: int, new_discount: UpdateProductDiscount, token_data: dict = dict()) -> None:
        """
        ...
        async with engine:
            ...
            if is_admin:
                ...
                is_updated: bool = await engine.product_repository.update_one(other_id=id_product, data_to_update=new_discount.model_dump())

                if is_updated:
                    return
                logging.info(msg=f"{ProductService.__name__} " f"Не удалось обновить скидку товара")
                (await ProductHttpError().http_failed_to_update_product_information())
            logging.info(msg=f"{ProductService.__name__} " f"Не удалось обновить скидку товара, " f"пользователь не был найден")
            await UserHttpError().http_user_not_found()

...
from src.api.core.ss_category_catalog.errors.sub_subcategory_exceptions import (
    SubSubCategoryException,
)
...
class SubSubCategoryService:
    ...
    async def create_new_sub_subcategory(
        ...
        async with engine:
            ...
            if is_admin and is_admin.id_type_user == 2:
                ...
                is_created = await engine.sub_subcategory_repository.add_one(
                    data=SubSubCategory(
                        name=new_s_sc.name,
    ...
    @redis
    @staticmethod
    async def get_sub_subcategory_by_id_s(
        engine: IEngineRepository, id_s: int, redis_search_data: str = ""
    ) -> AllSubSubCategory:
        """
            ...
            if all_sub_subcategory:
                return AllSubSubCategory(
                    all_sub_subcategories=[
                        SubSubCategoryBase(
                            name=model[0].name,
                            id_sub_category=model[0].id_sub_category,
                        )
                        for model in all_sub_subcategory
                    ]
                )
    ...
    @redis
    @staticmethod
    async def get_all_sub_subcategory(
        ...
        async with engine:
            ...
            all_sub_subcategory = (
                await engine.sub_subcategory_repository.find_all()
            )
            if all_sub_subcategory:
                return AllSubSubCategory(
                    all_sub_subcategories=[
    ...
    @auth(worker=AuthenticationEnum.DECODE_TOKEN.value)
    @staticmethod
    async def delete_sub_subcategory_by_id(
        engine: IEngineRepository,
        token: str,
        id_s_sc: int,
        token_data: dict = {},
        ...
        async with engine:
            ...
            if is_admin and is_admin.id_type_user == 2:
                is_deleted = await engine.sub_subcategory_repository.delete_one(
                    other_id=id_s_sc
                )
                if is_deleted:
                    return

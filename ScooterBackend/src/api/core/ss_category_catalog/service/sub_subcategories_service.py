from src.database.repository.sub_sub_categiry_repository import SubSubCategory
...
class SubSubCategoryService:
    ...
    @staticmethod
    async def create_new_sub_subcategory(engine: IEngineRepository, token: str, new_s_sc: SubSubCategoryBase, token_data: dict = {}) -> None:
        """
        Метод сервиса  SubSubCategoryService - создание новой
        под подкатегории товара
        """
        ...
        async with engine:
            ...
            if is_admin and is_admin.id_type_user == 2:
                ...
                is_created = await engine.sub_subcategory_repository.add_one(data=SubSubCategory(name=new_s_sc.name, id_sub_category=new_s_sc.id_sub_category))
    ...
    async def get_sub_subcategory_by_id_s(engine: IEngineRepository, id_s: int, redis_search_data: str = "") -> AllSubSubCategory:
        """
        Метод сервиса SubSubCategoryService - получение всех подкатегорий
        2-го уровня для подкатегории 1-го уровня
        """
        ...
        async with engine:
            ...
            if all_sub_subcategory:
                return AllSubSubCategory(all_sub_subcategories=[SubSubCategoryBase(name=model[0].name, id_sub_category=model[0].id_sub_category) for model in all_sub_subcategory])
    ...
    async def get_all_sub_subcategory(engine: IEngineRepository, redis_search_data: str = "") -> AllSubSubCategory:
        """
        Метод сервиса SubSubCategoryService - получение всех
        подкатегорий 2-го уровня
        """
        ...
        async with engine:
            ...
            if all_sub_subcategory:
                return AllSubSubCategory(all_sub_subcategories=[SubSubCategoryBase(name=model[0].name, id_sub_category=model[0].id_sub_category) for model in all_sub_subcategory])
            return AllSubSubCategory(all_sub_subcategories=[])
    ...
    async def delete_sub_subcategory_by_id(engine: IEngineRepository, token: str, id_s_sc: int, token_data: dict = {}) -> None:
        ...
        async with engine:
            ...
            is_admin = await engine.user_repository.find_user_by_email_and_password(email=token.get("email"))
            if is_admin and is_admin.id_type_user == 2:

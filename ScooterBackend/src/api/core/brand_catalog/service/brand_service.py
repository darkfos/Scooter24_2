from src.database.repository.brand_repository import BrandRepository, Brand
...
from src.api.core.brand_catalog.error.http_brand_exceptions import BrandException
...
class BrandService:
    ...
    @staticmethod
    async def add_a_new_brand(engine: IEngineRepository, token: str, new_brand: BrandBase, token_data: dict = dict()) -> NoReturn:
        """
        ...
        async with engine:
            ...
            if is_admin:
                ...
                new_brand = Brand(
                    name_brand=new_brand.name_brand
                )
    ...
    @redis
    @staticmethod
    async def get_brand_by_id(engine: IEngineRepository, id_brand: int, redis_search_data: dict = {}) -> BrandBase:
        """
    ...
    @redis
    @staticmethod
    async def get_all_brands(engine: IEngineRepository, redis_search_data: dict = {}) -> AllBrands:
        """
        ...
        async with engine:
            ...
            if brands:
                return AllBrands(
                    brands = [BrandBase(name_brand=brand[0].name_brand) for brand in brands]
                )
            return AllBrands(brands=[])
    
    @auth(worker=AuthenticationEnum.DECODE_TOKEN.value)
    @staticmethod
    async def delete_brand_by_id(engine: IEngineRepository, token: str, id_brand: int, token_data: dict = {}) -> None:
        """

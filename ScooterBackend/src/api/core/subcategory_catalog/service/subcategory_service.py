        ...
        async with engine:
            ...
            if is_admin:
                ...
                is_added = await engine.product_models_repository.add_one(
                    data=SubCategory(name=SubCategoryBase.name, id_category=SubCategoryBase.id_category)
                )
        ...
        async with engine:
            ...
            if all_models_by_id_category:
                return AllSubCategories(
                    all_subcategory=[SubCategoryBase(name=model[0].name, id_category=model[0].name) for model in all_models_by_id_category]
                )
    ...
    async def get_all_product_models(engine: IEngineRepository) -> AllSubCategories:
        ...
        async with engine:
            ...
            if all_subcategory_models:
                return AllSubCategories(
                    all_subcategory=[SubCategoryBase(name=model[0].name, id_category=model[0].name) for model in all_subcategory_models]
                )
            return AllSubCategories(
                all_subcategory=[]
            )

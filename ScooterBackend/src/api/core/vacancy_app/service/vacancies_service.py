...
class VacanciesService:
    ...
    async def get_all_vacancies(
        ...
        logging.info(msg=f"{VacanciesService.__name__} " f"Получение списка вакансий")
        async with engine:
        ...
        async with engine:
            ...
            if is_admin:
                ...
                if is_updated:
                    return
                logging.critical(
                    msg=f"{VacanciesService.__name__} " f"Не удалось обновить вакансию"
                )
        ...
        async with engine:
            ...
            if is_admin:
                ...
                if is_deleted:
                    return
                logging.critical(
                    msg=f"{VacanciesService.__name__} Не удалось" f" удалить вакансию"
                )

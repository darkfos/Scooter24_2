...
class UserService:
    ...
    async def create_a_new_user(
        ...
        async with engine:
            ...
            logging.critical(msg=f"{UserService.__name__} "
    ...
    async def get_information_about_me(
        ...
        logging.info(msg=f"{UserService.__name__} "
                         f"Получение информации пользователе")
        ...
        async with engine:
            ...
            if user_data:
                information = InformationAboutUser(
                    ...
                    if user_data.name_user else "",
                    surname_user=user_data.surname_user
                    if user_data.surname_user else "",
    ...
    async def get_information_about_me_and_history(
        ...
        logging.info(msg=f"{UserService.__name__} Получение информации о "
        ...
        async with engine:
            ...
            if is_admin:
                user_all_information: Union[None, User] = (
                    await engine.user_repository
                    .find_user_and_get_full_information(
                        user_id=user_id
                    )
                )
    ...
    async def user_is_created(
        ...
        logging.info(msg=f"{UserService.__name__} Поиск пользователя"
                         f" по почте и паролю")
        async with engine:
    ...
    async def update_user_information(
        ...
        logging.info(msg=f"{UserService.__name__} "
                         f"Обновление данных о пользователе")
    ...
    async def update_user_password(
        ...
        logging.info(msg=f"{UserService.__name__} "
                         f"Обновление пароля пользователя")
        crypt = CryptographyScooter()

        async with engine:
            # Проверка на совпадение пароля
            get_user_data: Union[User, None] = await engine.user_repository \
                .find_one(
                other_id=token_data.get("id_user")
            )
            if get_user_data:
    ...
    async def delete_user(
            engine: IEngineRepository,
            token: str,
            token_data: dict = dict()) -> UserIsDeleted:
        """
                    ...
                    if is_updated:
                        password_is_updated: bool = (
                            await engine.user_repository.update_one(
                                other_id=token_data.get("id_user"),
                                data_to_update={
                                    "password_user": CryptographyScooter(
                                    ).hashed_password(
                                        new_password
                                    )
                                },
                        ...
                        if password_is_updated:
                            return
            logging.critical(msg=f"{UserService.__name__} Не удалось "
    ...
    async def update_address_user_data(
        ...
        """
        logging.info(msg=f"{UserService.__name__} "
                         f"Обновление данных пользователя")

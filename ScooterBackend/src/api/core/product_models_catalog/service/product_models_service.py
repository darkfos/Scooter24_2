    ...
    async def added_new_model_to_product(engine: IEngineRepository, token: str, new_model: ProductModelsBase, token_data: dict = {}) -> None:
        ...
        async with engine:
            ...
            if is_admin:
                ...
                is_added = await engine.product_models_repository.add_one(data=ProductModels(id_product=new_model.id_product, id_model=new_model.id_model))
    ...
    async def get_model_by_id_product(engine: IEngineRepository, id_product: int, redis_search_data: str = "") -> AllProductModels:
        ...
        async with engine:
            ...
            if all_models_by_id_product:
                return AllProductModels(all_models=[ProductModelsBase(id_product=model[0].id_product, id_model=model[0].id_model) for model in all_models_by_id_product])
    ...
    async def get_all_product_models(engine: IEngineRepository) -> AllProductModels:
        ...
        async with engine:
            ...
            if all_product_models:
                return AllProductModels(all_models=[ProductModelsBase(id_product=model[0].id_product, id_model=model[0].id_model) for model in all_product_models])
            return AllProductModels(all_models=[])

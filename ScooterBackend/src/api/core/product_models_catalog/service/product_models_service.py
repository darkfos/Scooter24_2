...
class ProductModelsService:
        ...
        async with engine:
            ...
            if is_admin:
                ...
                is_added = await engine.product_models_repository.add_one(
                    data=ProductModels(
                        id_product=new_model.id_product,
                        id_model=new_model.id_model,
                    )
                )
    ...
    @redis
    async def get_model_by_id_product(
        ...
        async with engine:
            ...
            if all_models_by_id_product:
                return AllProductModels(
                    all_models=[
                        ProductModelsBase(
                            id_product=model[0].id_product,
                            id_model=model[0].id_model,
                        )
                        for model in all_models_by_id_product
                    ]
                )
            (
    ...
    async def delete_product_models_by_id(
        ...
        async with engine:
            ...
            if is_admin:
                ...
                is_deleted = await engine.product_models_repository.delete_one(
                    other_id=id_pr_m
                )

...
from src.api.core.user_catalog.service.user_service import UserService, UserHttpError
...
class EmailService:
    @staticmethod
    async def send_secret_key_for_register(engine: IEngineRepository, new_user: AddUser) -> None:
        """
        ...
        logging.info(msg=f"{EmailService.__name__} Отправка ссылки на подтверждение регистрации")
        secret_key: str = SecretKey().generate_password()

        async with engine:
            user_data = await engine.user_repository.find_user_by_email_and_password(email=new_user.email_user)
            if user_data:
                is_updated = await engine.user_repository.update_one(other_id=user_data.id, data_to_update={
                    "secret_create_key": secret_key
                })
                if is_updated:
                    await email_transfer.send_message(
                        text_to_message="Ваша ссылка для подтверждения регистрации, пожалуйста никому её не передавайте.\n" + \
                                        f"http://localhost:8000/auth/access_create_account?secret_key={secret_key}&email={new_user.email_user}",
    ...
    @staticmethod
    async def access_user_account(engine: IEngineRepository, user_email: str, secret_key: str) -> None:
        """
        Метод сервиса EmailService - подтверждение аккаунта пользователя
        """

        async with engine:
            user_data = await engine.user_repository.find_user_by_email_and_password(email=user_email)
    ...
    async def send_secret_key_by_update_password(
        ...
        logging.info(msg=f"{EmailService.__name__} Отправка секретного ключа для обновления пароля")

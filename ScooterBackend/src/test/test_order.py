...
async def test_get_all_orders(async_client: AsyncClient) -> None:
    req = await async_client.get(url="/v1/order/all/user")
...
async def test_get_unique_order(async_client: AsyncClient) -> None:
    req = await async_client.get(url="/v1/order/unique/3")
...
async def test_delete_order(async_client: AsyncClient) -> None:
    req = await async_client.delete(url="/v1/order/delete/3")
...
async def test_create_order(async_client: AsyncClient) -> None:
    req = await async_client.post(
        url="/v1/order/create",
        data={"id_product": 3, "date_create": datetime.date.today()},
    )

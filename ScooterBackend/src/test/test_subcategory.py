...
@pytest.mark.asyncio
async def test_get_all_subcategories_by_id_category(async_client: AsyncClient) -> None:
    req = await async_client.get(
        url="/api/v1/sub_category/all/category/2"
    )
...
async def test_delete_subcategory_by_id(async_client: AsyncClient) -> None:
    req = await async_client.delete(
        url="/api/v1/sub_category/delete/2"
    )

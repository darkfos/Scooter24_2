...
class GeneralSQLRepository:
    ...
    async def add_one(self, data: dict) -> bool:
        ...
        try:
            logging.info(msg=f"{self.model.__class__.__name__} "
            ...
            if result:
                ...
                logging.exception(msg=f"{self.model.__class__.__name__} "
    ...
    async def delete_one(
        ...
        res_to_del: int = (await self.async_session.execute(
            stmt
        )).rowcount
        if res_to_del:
            await self.async_session.commit()
            ...
            await self.async_session.rollback()
            logging.critical(msg=f"{self.model.__class__.__name__} "
